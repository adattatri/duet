    ...
    | `Sat interp -> begin
        ...
        if (!disjuncts) = (!opt_abstract_limit) then begin
          ...
          let disjunct =
            ...
            | None -> assert false
          in
          (* logf ~level: `always "first one\n%a\n" (Polyhedron.pp cs) disjunct; *)
          let lia_disjunct = perform_integer_hull disjunct in
          (* logf ~level: `always "second one\n%a\n" (Polyhedron.pp cs) lia_disjunct; *)
          let projected_coordinates =
            ...
            |> BatList.of_enum
          in
          (* List.iter (fun i -> print_int i; print_string " ") projected_coordinates; *)
          (* perform Fourier-Motzkin elimination *)
          let project1 = Polyhedron.project projected_coordinates lia_disjunct in
          (* logf ~level: `always "\nprojected polyhedron\n%a\n" (Polyhedron.pp cs) project1; *)
